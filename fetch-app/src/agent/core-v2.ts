/**
 * @fileoverview V2 Agent Core - Orchestrator Architecture
 *
 * The v2 agent is a lightweight orchestrator that:
 * 1. Classifies user intent
 * 2. Routes to appropriate tools
 * 3. Delegates coding work to harnesses
 *
 * @module agent/core-v2
 * @see {@link classifyIntentV2} - Intent classification
 * @see {@link V2ToolRegistry} - Tool registry
...
import { classifyIntentV2, type IntentTypeV2 } from './intent-v2.js';
import {
  buildOrchestratorPrompt,
  buildTaskFramePrompt,
} from './prompts-v2.js';
import { getV2Registry } from '../tools/v2/registry.js';
...
/**
 * Process a user message through the v2 agent
 *
...
export async function processMessageV2(
  ...
  try {
    ...
    // 1. Classify intent
    const intent = classifyIntentV2(message, session);
