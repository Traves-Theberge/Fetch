/**
 * @fileoverview V2 Intent Classification
 *
 * Simplified intent classification for the v2 orchestrator architecture.
...
/**
 * V2 Intent types (3 categories)
 */
export type IntentTypeV2 = 'conversation' | 'workspace' | 'task';

/**
 * Classification result
 */
export interface ClassifiedIntentV2 {
  type: IntentTypeV2;
  confidence: number;
  reason: string;
}
 ...
 */
export function classifyIntentV2(
  message: string,
  _session: Session
): ClassifiedIntentV2 {
  ...
  for (const pattern of CONVERSATION_PATTERNS) {
    if (pattern.test(trimmed)) {
      logger.debug('Intent v2: conversation', { message: trimmed.substring(0, 50) });
      return {
        type: 'conversation',
        confidence: 0.95,
  ...
  for (const pattern of WORKSPACE_PATTERNS) {
    if (pattern.test(trimmed)) {
      logger.debug('Intent v2: workspace', { message: trimmed.substring(0, 50) });
      return {
        type: 'workspace',
        confidence: 0.9,
  ...
  for (const pattern of TASK_PATTERNS) {
    if (pattern.test(trimmed)) {
      logger.debug('Intent v2: task', { message: trimmed.substring(0, 50) });
      return {
        type: 'task',
        confidence: 0.85,
        reason: 'task_pattern',
  ...
  if (trimmed.length < 20 && !hasCodeIndicators(lower)) {
    logger.debug('Intent v2: conversation (short message)', { message: trimmed });
    return {
      type: 'conversation',
      confidence: 0.6,
  ...
  logger.debug('Intent v2: task (default)', { message: trimmed.substring(0, 50) });
  return {
    type: 'task',
    confidence: 0.5,
    reason: 'default',
 ...
 */
export function parseIntentResponse(response: string): IntentTypeV2 | null {
 ...
 */
export function shouldUseLLMFallback(patternResult: ClassifiedIntentV2): boolean {
