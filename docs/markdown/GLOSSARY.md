...
| **LLM-First Architecture** | v4.0 design where every message (except 5 safety escapes) takes the same single path through the LLM with all 12 tools. No intent classification or conversation/action split. |
...
| Term | Definition |
|------|-----------|
| **Orchestrator Tool** | One of 12 tools the LLM can call during the ReAct loop: 6 workspace tools (`workspace_list`, `workspace_select`, `workspace_status`, `workspace_create`, `workspace_delete`, `workspace_sync`), 4 task tools (`task_create`, `task_status`, `task_cancel`, `task_respond`), and 2 interaction tools (`ask_user`, `report_progress`). |
