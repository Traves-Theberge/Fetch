...
## 🔬 Lab 8: GitHub Power User

**Goal:** Verify complex GitHub orchestrations.

**Prompt:**
> "Search GitHub for 'whatsapp-web.js', list the open PRs on the first repo that matches, and then create an issue on this current repo linking to that repo as a reference."

**Success Criteria:**

1. **Tool Flow:** `github_search_repos` → `github_pr_list` (on external repo) → `github_issue_create` (on current repo).
2. **Context:** The agent correctly identifies the PR count and the correct repo URL.
3. **Verification:** Check the newly created issue on your repository for the correct reference link.
